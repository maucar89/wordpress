...
class SalesOrder(models.Model):
    ...
    def single_importer(self, backend, sale_id, woo_id=None):
            ...
            for line in record['tax_lines']:
                ...
                if not tax_id:
                    ...
                    state_code = self.env['res.country.state'].search(
                        [('code', '=', tax['state']), ('country_id', '=', tax['country'])])
        ...
        if mapper:
            ...
            if 'line_items' in record:
                ...
                for details in product_ids:
                    ...
                    if order:
                        order.write(details)
            # if record['status']:
            #     sale_order_id.update({'status': record['status'].capitalize()})
            ...
            #     if record['shipping_lines']:
            #         delivery_method = record['shipping_lines'][0]['method_title']
            #         delivery_method_id = self.env['delivery.carrier'].search([('name','=',delivery_method)]).id
            # except:
            #     pass
