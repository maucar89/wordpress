...
class wp_configure(models.Model):
    ...
    def map_sale_order(self):
        ...
        # """ Assign backend to all the invoices orders """
        # try:
        #     all_invoices = self.env['account.invoice'].search(
        #         [('backend_id', '!=', self.id)])
        #     for invoice in all_invoices:
        #         backends = []
        #         for backend_id in invoice.backend_id:
        #             backends.append(backend_id.id)
        #         if not self.id in backends:
        #             backends.append(self.id)
        #         invoice.write({'backend_id': [[6, False, backends]]})
        # except:
        #     pass
        return True
    ...
    def export_invoices_refund(self):
        ...
        for account_invoice in all_invoice_orders:
            if 'RINV' in account_invoice.name or account_invoice.type == "out_refund":
