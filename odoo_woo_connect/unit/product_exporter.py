...
class WpProductExport(WpImportExport):
    ...
    def get_images(self, product):
        ...
        if product.image_medium:
            ...
            with open(os.path.expanduser(file_path), 'wb') as fout:
                fout.write(base64.decodestring(product.image_medium))
            ...
            if not wp_image_url:
                ...
                res = requests.post(url='{}/wp-json/wp/v2/media'.format(self.backend.location),
                                    data=data,
                                    headers={'Content-Type': 'image/jpg',
        ...
        for var_ids in product.product_variant_ids:
            ...
            if woo_product_comb:
                ...
                product_variant_dict = {
                    ...
                    "sale_price": woo_product_comb.list_price or None,
    ...
    def export_product_variant(self, product, res_data, backend):
        ...
        mapper = product.backend_mapping.search(
            [('backend_id', '=', backend.id), ('product_id', '=', product.id)])
        ...
        if mapper and (res_data['status'] == 200 or res_data['status'] == 201):
            ...
            product.write({'default_code': res_data['data']['sku'], 'slug': res_data['data']['slug']})
            product.backend_mapping.create(
                {'product_id': product.id, 'backend_id': backend.id, 'woo_id': res_data['data']['id']})
            ...
            if woo_product_comb:
                ...
                if product_variant_dict:
                    ...
                    if product_variant_dict['id']:
                        record_data = wcapi.put("products/{}/variations/{}".format(product_templ_mapper_woo_id,
                                                                                   product_variant_dict['id']),
                                                product_variant_dict)
                    else:
                        record_data = wcapi.post("products/{}/variations".format(product_templ_mapper_woo_id),
                                                 product_variant_dict)
    ...
    def product_variant_map(self, variant, backend, record_response):
        ...
        if mapper and (record_response['status'] == 200 or record_response['status'] == 201):
            ...
            mapper.write(
                {'product_id': variant.id, 'backend_id': backend.id, 'woo_id': record_response['data']['id']})

        elif (record_response['status'] == 200 or record_response['status'] == 201):
            # variant.write({'default_code': record_response['data']['sku']})
            variant.backend_mapping.create(
                {'product_id': variant.id, 'backend_id': backend.id, 'woo_id': record_response['data']['id']})