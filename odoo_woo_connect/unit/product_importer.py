...
class WpProductImport(WpImportExport):
	...
	def create_product(self,backend,mapper,res,status=True):
		if (res['status'] == 200 or res['status'] == 201):
			...
			try :
				...
				# 	categ_id = mapper.env['product.category'].search([('name','=',woo_category)]).id
				# 	if not categ_id :
				# 		categ_id = mapper.env['product.category'].search([('name','=','All')]).id
				if res['data']['categories']:
					woo_category = res['data']['categories'][0]['name']
			...
			if res['data']['variations']:
				...
				for record in res['data']['variations']:
					...
					# 	record_img = record_data['image']['src']
					# 	# response = requests.get(img)

					# 	headers={'User-Agent': 'Mozilla/5.0'}
					# 	response = requests.get(record_img, headers=headers)

					# 	image = Image.open(BytesIO(response.content))
					# 	imgByteArr = io.BytesIO()
					# 	image.save(imgByteArr, format='PNG')
					# 	imgByteArr = imgByteArr.getvalue()
					# 	record_image = base64.b64encode(imgByteArr)
					# else:
					# 	record_image = None
					...
					for variant_id in product.product_variant_ids:
						...
						for product_template_attribute_value_id in variant_id.attribute_value_ids:
						...
						if set(variant_attr_lst) == set(record_attr_lst):
							...
							if variant_mapper:
								...
								stock_quant.with_context(inventory_mode=True).create({
									...
									'qty': record_data['stock_quantity'],
								})
								break
	...
	def get_tags(self, wp_tags, mapper,bkend_id):
		...
		for wp_tag in wp_tags:
			...
			if product_tag:
				...
				tag_list.append(product_tag.id)
			if product_tag:
				tag_mapper = product_tag.backend_mapping.search([('backend_id','=',bkend_id.id),('product_tag_id','=',product_tag.id)], limit=1)

			if tag_mapper:
				tag_mapper.write({'product_tag_id': product_tag.id, 'backend_id': bkend_id.id, 'woo_id': wp_tag['id']})
			else:
				product_tag.backend_mapping.create({'product_tag_id': product_tag.id, 'backend_id': bkend_id.id, 'woo_id': wp_tag['id']})
	...
	def write_product(self, backend, mapper, res):
		...
		if res['data']['attributes']:
			...
			attributes_value = []
			for attribute in res['data']['attributes']:
				attribute_id = mapper.env['product.attribute'].search([('name', '=', attribute['name'])],limit=1).id
				attribute_value_ids = []
		...
		# if mapper.product_id.product_variant_ids or mapper.product_id.product_attribute_line_ids:
		# 	for variant_id in mapper.product_id.product_variant_ids:
		# 		variant_id.weight = res['data']['weight']
		# 		variant_id.default_code = res['data']['sku']
		# else:
		# 	mapper.product_id.weight = res['data']['weight']
		# 	mapper.product_id.weidefault_code = res['data']['sku']
		...
		# 				'product_id': variant_id.id,
		# 				'location_id': warehouse.lot_stock_id.id,
		# 				'inventory_quantity': res['data']['stock_quantity'],
		# 			})
